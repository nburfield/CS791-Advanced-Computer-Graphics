...
void createMesh(vector<vector<float>>& input, float xres, float yres, float max, vector<int>& indicies, vector<BasicVertex>& vertexes)
{

  // Our vertex information along with normals contained inside
  std::vector<BasicVertex> vectors = vector<BasicVertex>();
  ...
  for (int i = 0; i < input.size() - 1; i++)
  {
    for (int j = 0; j < input[i].size() - 1; j++)
    {
      ...
      vectors.push_back(BasicVertex(glm::vec3(i * xres, UL, j * yres), glm::vec2((float)i / (float)input.size(), (float)j / (float)input[i].size()), a));
      vectors.push_back(BasicVertex(glm::vec3((i + 1)*xres, LL, j * yres), glm::vec2((float)(i + 1) / (float)input.size(), (float)j / (float)input[i].size()), b));
      vectors.push_back(BasicVertex(glm::vec3(i * xres, UR, (j + 1)*yres), glm::vec2((float)i / (float)input.size(), (float)(j + 1) / (float)input[i].size()), c));
      vectors.push_back(BasicVertex(glm::vec3((i + 1)*xres, LR, (j + 1)*yres), glm::vec2((float)(i + 1) / (float)input.size(), (float)(j + 1) / (float)input[i].size()), d));
