...
bool NullTechnique::Init()
{
  ...
  if(!vertex.Initilize("../shader/null_technique_vertex.glsl", GL_VERTEX_SHADER, program))
  {
    return false;
  }
  std::cout<<"Null Technique Vertex Shader Success"<<std::endl;

  if(!fragment.Initilize("../shader/null_technique_fragment.glsl", GL_FRAGMENT_SHADER, program))
  {
    return false;
  }
  std::cout<<"Null Technique Fragment Shader Success"<<std::endl;
