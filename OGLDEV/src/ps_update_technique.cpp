...
bool PSUpdateTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, "../shaders/ps_update.vs")) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, "../shaders/ps_update.gs")) {
        return false;
    }

  if (!AddShader(GL_FRAGMENT_SHADER, "../shaders/ps_update.fs")) {
    return false;
  }
